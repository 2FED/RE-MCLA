...
The first status line must contain ReXGlue project-fork commit `071a80d17b9a629c7070880008a7bff1ac928c70`; no line may begin with `-`, `+`, or `U`.

The verified v0.9.0.13 Windows build uses:

- ReXGlue 0.9.0.13 project fork, based on upstream v0.9.0, with the exact nested dependencies in `docs/rexglue-sdk.md`
...
The presets pin the installed project-fork SDK at ReXGlue `0.9.0.13`. After successful non-force codegen, verify the ignored corpus, configure, build, and verify the Release integration with:
