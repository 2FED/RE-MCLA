...
- `CMakeLists.txt` pins ReXGlue 0.9.0.21, validates the contained 65-source generated graph, owns the host target, and invokes `rexglue_setup_target(mcla GPU_PLUGINS xenos)` so every configuration stages the matching runtime-loaded Xenos plugin
...
creation. ReXGlue v0.9.0.21 supplies the fail-closed device boundary and explicit
...
invent one. ReXGlue v0.9.0.21 canonicalizes only the relative suffix when
