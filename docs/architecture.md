...
- `CMakeLists.txt` pins ReXGlue 0.9.0.26, validates the contained 65-source generated graph, owns the host target, and invokes `rexglue_setup_target(mcla GPU_PLUGINS xenos)` so every configuration stages the matching runtime-loaded Xenos plugin
...
creation. ReXGlue v0.9.0.26 supplies the fail-closed device boundary and explicit
...
ReXGlue v0.9.0.26 extends that boundary to the return-bearing progression
...
invent one. ReXGlue v0.9.0.26 canonicalizes only the relative suffix when
...
v0.9.0.26 stores only known standard XAM settings under the local user's global
