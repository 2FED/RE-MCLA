...
- `CMakeLists.txt` pins ReXGlue 0.9.0.30, validates the contained 65-source generated graph, owns the host target, and invokes `rexglue_setup_target(mcla GPU_PLUGINS xenos)` so every configuration stages the matching runtime-loaded Xenos plugin
...
creation. ReXGlue v0.9.0.30 supplies the fail-closed device boundary and explicit
...
ReXGlue v0.9.0.30 extends that boundary to the return-bearing progression
...
invent one. ReXGlue v0.9.0.30 canonicalizes only the relative suffix when
resolving mounted VFS roots, allowing saved-profile root opens with trailing
separators without changing containment.

ReXGlue v0.9.0.30 separates stable SDL physical storage slots from the guest's
...
v0.9.0.30 stores only known standard XAM settings under the local user's global
