...
git -C third_party/rexglue-sdk rev-parse refs/tags/v0.9.0.12^{}
git submodule status --recursive
git -C third_party/rexglue-sdk status --short --ignore-submodules=none
```

The first two commands must return `b44f69149c079f4c3b62503084449d7a7f5e4412`. The upstream base remains available as `refs/tags/v0.9.0` at `3eb9b511b4140d2769e27be63eae57d41bfa2afa`. Every `git submodule status --recursive` line must begin with one space: `-` means uninitialized, `+` means a commit mismatch, and `U` means a merge conflict.

## Recursive SHA manifest

The nested dependency SHAs were captured from the clean upstream v0.9.0 checkout on 2026-08-10; the root line is the reviewed v0.9.0.12 project-fork commit:

```text
b44f69149c079f4c3b62503084449d7a7f5e4412 third_party/rexglue-sdk
