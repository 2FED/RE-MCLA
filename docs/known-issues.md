...
| KI-004 | S2 | Open | MCLA-R currently requires its pinned ReXGlue v0.9.0.4 fork for valid FLOAT16_4 mask-3 diagnostics, Unicode-safe Windows paths, safe input/window teardown, and fail-closed read-only VFS behavior. | The submodule pin is automatic; do not replace it with upstream v0.9.0 or unpinned `main`. | SDK/upstream |
