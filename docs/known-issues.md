...
| KI-011 | S1 | Open | The current normal native route reaches module launch with no GPU emulation configured, then ignores graphics callback/ring-buffer initialization before any observed post-launch Bink access. | Do not add skip-intro yet; exercise the intended GPU plugin in M3-012/M3-013 and re-evaluate Bink only after graphics initialization advances. | M3-012/M3-013 |
