...
| KI-011 | S1 | Open | All three native configurations now stage the matching Xenos GPU plugin DLL, but the normal route still needs the runtime `gpu_plugin=xenos` selection exercised before graphics callback/ring-buffer initialization can advance. | Do not add skip-intro yet; configure and trace the staged plugin in M3-013, then re-evaluate Bink only after graphics initialization advances. | M3-013 |
