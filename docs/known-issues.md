...
| KI-004 | S2 | Open | MCLA-R currently requires its pinned ReXGlue v0.9.0.5 fork for valid FLOAT16_4 mask-3 diagnostics, Unicode-safe Windows paths, safe input/window teardown, fail-closed read-only VFS behavior, and deterministic direct offline-service results. | The submodule pin is automatic; do not replace it with upstream v0.9.0 or unpinned `main`. | SDK/upstream |
...
| KI-007 | S2 | Open | Generic ReXGlue `REX_EXPORT_STUB` preserves caller `r3` for return-bearing exports. MCLA-R's ten direct offline-service imports are fixed, but the broader SDK inventory remains unclassified. | Keep the exact v0.9.0.5 fork pin; do not add a return-bearing generic stub. | SDK/upstream #407 |
