...
MCLA-R has completed its clean native build and module-boot milestone and is active in M4. Version `0.3.0.0` reliably loads the exact supported image. M4-001 verifies guest-backed D3D12 presentation/readback, and M4-002 now verifies ten autonomous reaches to the Complete Edition title frontend with bounded render-path telemetry and a private Xenia UI-region comparison. Menu navigation, input/profile behavior, frontend audio, gameplay, and public packaging are not yet validated, so this is not a playable build or public game release.
