...
MCLA-R has completed its clean native build and module-boot milestone and is active in M4. Version `0.3.0.0` reliably loads the exact supported image. M4-001 verifies guest-backed D3D12 presentation/readback, M4-002 verifies ten autonomous reaches to the Complete Edition title frontend with bounded render-path telemetry and a private Xenia UI-region comparison, and M4-004 verifies the reached single-local-user XAM identity contract. Menu navigation, controller input, frontend audio, gameplay, profile persistence, and public packaging are not yet validated, so this is not a playable build or public game release.
