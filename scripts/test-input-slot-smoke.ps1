...
function Get-SourceText {$parts=@();foreach($n in 3,2,1){$p=Join-Path $source "mcla.$n.log";if(Test-Path $p){$parts+=[IO.File]::ReadAllText($p)}};$parts+=[IO.File]::ReadAllText((Join-Path $source 'mcla.log'));$parts-join[Environment]::NewLine}
