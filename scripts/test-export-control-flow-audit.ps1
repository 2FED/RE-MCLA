...
try {
    & $exporter -AuditPath $audit -ForceRoot $forceRoot -GeneratedRoot $generatedRoot -OutputPath $output | Out-Null
    ...
    Assert-Rejected 'missing-record' { & $exporter -AuditPath $missing -ForceRoot $forceRoot -GeneratedRoot $generatedRoot -OutputPath $output | Out-Null }
    ...
    Assert-Rejected 'changed-terminal' { & $exporter -AuditPath $changed -ForceRoot $forceRoot -GeneratedRoot $generatedRoot -OutputPath $output | Out-Null }

    Assert-Rejected 'outside-output' { & $exporter -AuditPath $audit -ForceRoot $forceRoot -GeneratedRoot $generatedRoot -OutputPath (Join-Path $testRoot 'outside.md') | Out-Null }
