...
$gameplaySeedRelative='private/save-archive/M6-014/20260829-172903-5bc0bc1e/free-roam/20260829-224025Z-FFB227E877B9A8B4-A5A662F728AB46D8'
$gameplaySeedUpstreamRelative="$gameplaySeedRelative/snapshot.json"
$gameplaySeedUpstreamSha256='38C2005BEF85665A463A2D8922FD920F9028026E6C229BE10F9B98C0B465DB29'
$gameplaySeedSaveSha256='FFB227E877B9A8B4F5011D82234DCAF146487A9DFDF60611A9F434C54147364E'
...
if($gameplayRaceCompletions-ne35-or$gameplayWins-ne29){throw "Progressed gameplay seed statistics drifted: expected 35 races / 29 wins, got $gameplayRaceCompletions / $gameplayWins."}
...
$expectedSeeds=@(
  ...
  (SeedRecord 'gameplay' $gameplaySeedRelative 'owner-progressed-save-35-races-29-wins' $gameplaySeedUpstreamRelative $gameplaySeedUpstreamSha256 $gameplaySeed)
)
...
if(-not(Test-Path $suitePath)){
  ...
  if(-not$seedLineageMatches){
    if(-not$RebindGameplaySeed){throw 'Explicit seed lineage drifted. Stop the old attempt and use -RebindGameplaySeed to preserve it and bind the progressed 35-race / 29-win save.'}
    ...
    $record=[ordered]@{schema='mcla-soak-gameplay-seed-rebind-v1';rebound_utc=[DateTime]::UtcNow.ToString('O');reason='replace-prior-gameplay-seed-with-owner-progressed-35-race-29-win-seed';old_seed=$oldGameplay[0];new_seed=$expectedSeeds[1];aborted_attempts=@($aborted)}
...
if($RebindGameplaySeed){Write-Host "M6-014 gameplay seed REBOUND: 35 races / 29 wins. Preserved old attempt under '$suiteRoot/aborted-attempts'." -ForegroundColor Green;return}
