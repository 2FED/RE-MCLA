...
$gameplaySeedRelative='private/save-archive/M6-014/20260831-133236-2cecb67b/free-roam/20260901-114045Z-A575F88F4FDFA19B-2FAEFBC7FF8CDEAD'
$gameplaySeedUpstreamRelative="$gameplaySeedRelative/snapshot.json"
$gameplaySeedUpstreamSha256='BE2D1D6C79DC834A0E5C50A601667A51E10EA86AF5E16062380ACFC2DEAEE79B'
$gameplaySeedSaveSha256='A575F88F4FDFA19B084BA3C5DBA3B4A15EBFBDCCBD6CCD09628201A9B84A6F82'
$gameplaySeedHeaderSha256='2FAEFBC7FF8CDEADBCE025D9BE914436ED05E27D041F72C8C88798612CFB0D60'
...
function HostDisplayAudit([DateTime]$Since){
  try{
    $null=Get-WinEvent -ListLog System -ErrorAction Stop;$null=Get-WinEvent -ListLog Application -ErrorAction Stop
    $nvidia=@(Get-WinEvent -FilterHashtable @{LogName='System';StartTime=$Since;Id=153} -ErrorAction SilentlyContinue|Where-Object{$_.ProviderName-ceq'nvlddmkm'}|ForEach-Object{[ordered]@{provider=$_.ProviderName;event_id=[int]$_.Id;time_created_utc=$_.TimeCreated.ToUniversalTime().ToString('O')}})
    $sunshine=@(Get-WinEvent -FilterHashtable @{LogName='Application';StartTime=$Since;Id=1000} -ErrorAction SilentlyContinue|Where-Object{$_.ProviderName-ceq'Application Error'-and(($_.Properties.Count-gt0-and[string]$_.Properties[0].Value-ceq'Sunshine.exe')-or$_.Message-match'(?i)Sunshine\.exe')}|ForEach-Object{[ordered]@{provider=$_.ProviderName;event_id=[int]$_.Id;time_created_utc=$_.TimeCreated.ToUniversalTime().ToString('O');application='Sunshine.exe';exception_code=if($_.Properties.Count-gt6){[string]$_.Properties[6].Value}else{'unavailable'}}})
    [ordered]@{schema='mcla-host-display-audit-v1';since_utc=$Since.ToUniversalTime().ToString('O');checked_utc=[DateTime]::UtcNow.ToString('O');audit_available=$true;nvidia_driver_errors=@($nvidia);sunshine_application_errors=@($sunshine)}
  }catch{
    [ordered]@{schema='mcla-host-display-audit-v1';since_utc=$Since.ToUniversalTime().ToString('O');checked_utc=[DateTime]::UtcNow.ToString('O');audit_available=$false;audit_error=$_.Exception.Message;nvidia_driver_errors=@();sunshine_application_errors=@()}
  }
}
function Assert-HostDisplayHealthy([DateTime]$Since,[string]$Path){$audit=HostDisplayAudit $Since;WriteJson $Path $audit;if(-not$audit.audit_available){throw 'Host display event logs could not be audited.'};if(@($audit.nvidia_driver_errors).Count){throw "Host NVIDIA display-driver failure detected; title evidence is invalid. Diagnostic: '$Path'."};if(@($audit.sunshine_application_errors).Count){throw "Host Sunshine streaming-process failure detected; title evidence is invalid. Diagnostic: '$Path'."}}
...
function Read-ProcessAwareLine([Diagnostics.Process]$Process,[string]$Prompt){
  try{$null=[Console]::KeyAvailable}catch{throw 'Interactive console input is unavailable; run the soak in a visible console.'}
  Write-Host ($Prompt+': ') -NoNewline;$input=[Text.StringBuilder]::new()
  while($true){
    if($Process.HasExited){Write-Host;throw "Process exited while waiting for operator input (exit $($Process.ExitCode))."}
    if(-not[Console]::KeyAvailable){Start-Sleep -Milliseconds 50;continue}
    $key=[Console]::ReadKey($true)
    if($key.Key-eq[ConsoleKey]::Enter){Write-Host;return $input.ToString()}
...
function NextBoundary([int]$Elapsed,[int]$Interval){([Math]::Floor([double]$Elapsed/$Interval)+1)*$Interval}
function PromptReady([Diagnostics.Process]$Process,[string]$Name){if($Name-eq'frontend'){return};$instructions=@{'free-roam'='Saved gameplay should now be loaded. Play normally: races and story progression are allowed; keep moving through the city.';'races'='Saved gameplay should now be loaded. Begin completing race events.';'garage'='Saved gameplay should now be loaded. Open the garage/customization loop.';'lifecycle'='Saved gameplay should now be loaded. Begin pause/focus/minimize/controller lifecycle cycles.'};$exact="SOAK $($Name.ToUpperInvariant()) READY";Write-Host $instructions[$Name] -ForegroundColor Yellow;do{$answer=Read-ProcessAwareLine $Process "Type exactly: $exact"}while($answer-cne$exact)}
function PromptActivity([Diagnostics.Process]$Process,[string]$Name,[int]$Checkpoint){
  switch($Name){
    'free-roam'{do{$v=Read-ProcessAwareLine $Process "15-minute checkpoint $Checkpoint/8: type FREEROAM <REGION-LABEL>"}while($v-cnotmatch'^FREEROAM ([A-Z0-9-]{2,40})$');[ordered]@{primary=1;secondary=0;label=$Matches[1]}}
    'races'{do{$v=Read-ProcessAwareLine $Process "15-minute checkpoint $Checkpoint/8: type RACES <completed> <series-completed>"}while($v-cnotmatch'^RACES ([0-9]{1,2}) ([0-9]{1,2})$');[ordered]@{primary=[int]$Matches[1];secondary=[int]$Matches[2];label='race-events'}}
    'garage'{do{$v=Read-ProcessAwareLine $Process "15-minute checkpoint $Checkpoint/8: type GARAGE <entries> <committed-changes>"}while($v-cnotmatch'^GARAGE ([0-9]{1,2}) ([0-9]{1,2})$');[ordered]@{primary=[int]$Matches[1];secondary=[int]$Matches[2];label='garage-cycles'}}
    'lifecycle'{do{$v=Read-ProcessAwareLine $Process "15-minute checkpoint $Checkpoint/8: type LIFECYCLE <complete-cycles> [LONG]"}while($v-cnotmatch'^LIFECYCLE ([0-9]{1,2})( LONG)?$');[ordered]@{primary=[int]$Matches[1];secondary=if($Matches[2]){1}else{0};label='lifecycle-cycles'}}
...
function Recover-CompletedScenario([string]$Name,$Suite,[string]$SuitePath,[string]$SuiteRoot,[string]$Seed,[string]$Exe){
  ...
  $primary=[int](($activity|ForEach-Object{[int]$_['primary']}|Measure-Object -Sum).Sum);$secondary=[int](($activity|ForEach-Object{[int]$_['secondary']}|Measure-Object -Sum).Sum);$labels=@($activity|ForEach-Object{[string]$_['label']}|Sort-Object -Unique)
...
if($gameplayRaceCompletions-ne37-or$gameplayWins-ne31){throw "Progressed gameplay seed statistics drifted: expected 37 races / 31 wins, got $gameplayRaceCompletions / $gameplayWins."}
...
$expectedSeeds=@(
  ...
  (SeedRecord 'gameplay' $gameplaySeedRelative 'owner-progressed-save-37-races-31-wins' $gameplaySeedUpstreamRelative $gameplaySeedUpstreamSha256 $gameplaySeed)
)
...
if(-not$versionPin.Success-or-not$commitPin.Success-or-not$tagPin.Success-or$tagPin.Groups['t'].Value-cne('v'+$versionPin.Groups['v'].Value)){throw 'Project SDK pins are missing or inconsistent.'}
$sdkTagOutput=@(&git -C $sdk describe --tags --exact-match HEAD 2>$null);if($LASTEXITCODE-ne0-or$sdkTagOutput.Count-ne1){throw 'M6-014 requires an exact SDK tag.'};$sdkTag=$sdkTagOutput[0].Trim();$sdkCommitOutput=@(&git -C $sdk rev-parse HEAD);if($LASTEXITCODE-ne0-or$sdkCommitOutput.Count-ne1){throw 'M6-014 could not resolve the SDK commit.'};$sdkCommit=$sdkCommitOutput[0].Trim();$sdkDirty=@(&git -C $sdk status --porcelain);if($LASTEXITCODE-ne0-or$sdkDirty.Count-or$sdkTag-cne$tagPin.Groups['t'].Value-or$sdkCommit-cne$commitPin.Groups['c'].Value){throw 'M6-014 requires the clean exact project-pinned SDK tag and commit.'};$sdkVersion=$versionPin.Groups['v'].Value
...
if(-not$InitializeOnly-and-not$Finalize-and-not$RecoverCompletedScenario-and-not$RecoverMixedRaceCoverage-and-not$RebindGameplaySeed){Assert-HostDisplayHealthy ([DateTime]::Now.AddMinutes(-1)) (Join-Path $suiteRoot 'host-display-preflight.json')}
if(-not(Test-Path $suitePath)){
  ...
  if(-not$seedLineageMatches){
    if(-not$RebindGameplaySeed){throw 'Explicit seed lineage drifted. Stop the old attempt and use -RebindGameplaySeed to preserve it and bind the progressed 37-race / 31-win save.'}
    ...
    $record=[ordered]@{schema='mcla-soak-gameplay-seed-rebind-v1';rebound_utc=[DateTime]::UtcNow.ToString('O');reason='replace-prior-gameplay-seed-with-owner-progressed-37-race-31-win-save-after-delivery-transition-crash';old_seed=$oldGameplay[0];new_seed=$expectedSeeds[1];aborted_attempts=@($aborted)}
...
if($RebindGameplaySeed){Write-Host "M6-014 gameplay seed REBOUND: 37 races / 31 wins. Preserved old attempt under '$suiteRoot/aborted-attempts'." -ForegroundColor Green;return}
...
if($RecoverMixedRaceCoverage){
  ...
  try{$process=Start-Process $exe -ArgumentList $args -WorkingDirectory $build -PassThru;if($Scenario-cne'frontend'){$archive=Safe "private/save-archive/M6-014/$SuiteRun/$Scenario" 'Progress archive';$watcher=Start-Process $pwshHost -ArgumentList @('-NoLogo','-NoProfile','-ExecutionPolicy','Bypass','-File',$saveWatcher,'-SourceUserRoot',$user,'-ArchiveRoot',$archive,'-MclaProcessId',$process.Id) -WorkingDirectory $repo -WindowStyle Hidden -PassThru;Write-Host "SAVE WATCHER       | ACTIVE - recoverable profile snapshots: $archive" -ForegroundColor Green};WaitMarker $process $runRoot 'MCLA graphics: nontrivial guest frame captured' 90;if($Scenario-cne'frontend'){Enter-Gameplay $process $runRoot $user};PromptReady $process $Scenario;$timer=[Diagnostics.Stopwatch]::StartNew();$samples+=Sample $process 0 0;$captureRecords+=Capture $process (Join-Path $captures 'checkpoint-00.bmp');WriteJson (Join-Path $scenarioRoot 'resource-samples.json') @($samples);WriteJson (Join-Path $scenarioRoot 'activity.json') @($activity)
    ...
    while($timer.Elapsed.TotalSeconds-lt$DurationSeconds){if($process.HasExited){Assert-HostDisplayHealthy $hostDisplayStart $hostDisplayPath;throw "Process exited during '$Scenario' soak."};$elapsed=[int][Math]::Floor($timer.Elapsed.TotalSeconds)
      ...
      if($elapsed-ge$nextCapture){$checkpoint=$captureIndex;if($Scenario-ne'frontend'){$activity+=PromptActivity $process $Scenario $checkpoint;WriteJson (Join-Path $scenarioRoot 'activity.json') @($activity)};if($process.HasExited){throw "Process exited before checkpoint $checkpoint capture."};$captureRecords+=Capture $process (Join-Path $captures ("checkpoint-{0:D2}.bmp"-f$captureIndex));$captureIndex++;$elapsed=[int][Math]::Floor($timer.Elapsed.TotalSeconds);$nextSample=NextBoundary $elapsed $sampleInterval;$nextCapture=NextBoundary $elapsed $captureInterval;$nextStatus=NextBoundary $elapsed 60;WriteJson (Join-Path $scenarioRoot 'captures.json') @($captureRecords)}
      Start-Sleep -Milliseconds 250
    }
    if($samples.Count-ne24-or$captureRecords.Count-ne8-or($Scenario-ne'frontend'-and$activity.Count-ne7)){throw 'Interactive delay caused scheduled soak evidence to be missed; refusing to fabricate catch-up samples or captures.'};$elapsed=[int][Math]::Floor($timer.Elapsed.TotalSeconds);$samples+=Sample $process $sampleIndex $elapsed;if($Scenario-ne'frontend'){$activity+=PromptActivity $process $Scenario 8};if($process.HasExited){throw 'Process exited before final soak capture.'};$captureRecords+=Capture $process (Join-Path $captures 'checkpoint-08.bmp');WriteJson (Join-Path $scenarioRoot 'resource-samples.json') @($samples);WriteJson (Join-Path $scenarioRoot 'captures.json') @($captureRecords);WriteJson (Join-Path $scenarioRoot 'activity.json') @($activity);Assert-HostDisplayHealthy $hostDisplayStart $hostDisplayPath
  ...
  $timer.Stop();$set=LogSet $runRoot;$fatal=[regex]::Matches($set.text,'(?i)\[FATAL\]|guest crash|PPC_UNIMPLEMENTED|invalid or unregistered function|device lost|DXGI_ERROR_DEVICE_REMOVED').Count;if($fatal){throw "Runtime fatal markers found after '$Scenario'."};$primary=if($activity.Count){[int](($activity|ForEach-Object{[int]$_['primary']}|Measure-Object -Sum).Sum)}else{0};$secondary=if($activity.Count){[int](($activity|ForEach-Object{[int]$_['secondary']}|Measure-Object -Sum).Sum)}else{0};$labels=@($activity|ForEach-Object{[string]$_['label']}|Sort-Object -Unique);if($Scenario-eq'frontend'){$labels=@('title-frontend')}
