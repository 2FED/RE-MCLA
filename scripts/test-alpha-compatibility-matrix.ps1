...
try {
    $positive = & $verify
    if (-not $positive.Passed -or $positive.CompatibilityRows -ne 13 -or $positive.KnownIssues -ne 26) { throw 'Positive matrix verification failed.' }
    $negative = 0
    ...
    [pscustomobject]@{ PositiveFixtures = 1; FailClosedNegatives = $negative; CompatibilityRows = 13; KnownIssues = 26; Passed = $true }
}
finally {
