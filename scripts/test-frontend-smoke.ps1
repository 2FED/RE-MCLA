...
try {
    $probe = & $verify -ProbeOnly -FixtureMode -RuntimeLogPath $log -UserRoot $user
    if (-not $probe.Passed -or $probe.PauseCorrelationPpm -lt 550000 -or $probe.OptionsCorrelationPpm -lt 550000) { throw 'Positive frontend fixture failed.' }
    $cases = [ordered]@{
    ...
    foreach ($case in $cases.GetEnumerator()) {
        ...
        try { $null = & $verify -ProbeOnly -FixtureMode -RuntimeLogPath $log -UserRoot $user } catch { $failed++; continue }
    ...
    $sources = [ordered]@{
        ...
        'gameplay-wait' = @('src/mcla_app.cpp', 'std::chrono::seconds(30)')
        ...
        'runner-three-cycles' = @('scripts/run-frontend-smoke.ps1', 'for ($index = 1; $index -le 3; $index++)')
