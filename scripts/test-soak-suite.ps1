...
try{
  ...
  foreach($needle in @('scenarioNames','duration_seconds_per_scenario','sample_count','capture_count','Free-roam region diversity','resource growth exceeds bounds','capture sequence lacks temporal variation','runtime health contract failed','if($s.name-ceq''frontend''){480}else{900}','SDK identity drifted','Canonical MCLA process remains')){if(-not$verifierText.Contains($needle)){throw "Verifier source contract missing: $needle"}}
