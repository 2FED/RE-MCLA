...
$sdkVersionPin=$versionPin.Groups['v'].Value;$sdkCommitPin=$commitPin.Groups['c'].Value;$wrongSdkVersion=if($sdkVersionPin-cne'99.99.99.99'){'99.99.99.99'}else{'98.98.98.98'};$wrongSdkCommit=if($sdkCommitPin-cne('0'*40)){('0'*40)}else{('1'*40)}
...
function BaseResult{
  ...
  for($i=0;$i-lt5;$i++){$name=$names[$i];$seedClass=if($name-eq'frontend'){'frontend'}else{'gameplay'};$labels=if($name-eq'free-roam'){@('HOLLYWOOD','BEVERLY','SANTA-MONICA','DOWNTOWN','CRENSHAW')}elseif($name-eq'races'){@('race-events')}else{@('title-frontend')};$saveBefore=if($name-eq'frontend'){('C'*64)}else{('D'*64)};$saveAfter=if($name-eq'frontend'){$saveBefore}else{('E'*64)};$headerBefore=if($name-eq'frontend'){('8'*64)}else{('9'*64)};$headerAfter=if($name-eq'frontend'){$headerBefore}else{('A'*64)};$stagePath=if($name-eq'races'){'scenarios/free-roam/race-coverage.json'}else{"scenarios/$name/stage.json"};$scenarios+=[ordered]@{name=$name;seed_class=$seedClass;decision="two-hour-$name-soak-pass";duration_seconds=7201L;sample_count=25;capture_count=9;activity_primary=$primary[$i];activity_secondary=$secondary[$i];distinct_labels=$labels;resource_bounds=[ordered]@{private_growth_bytes=100;working_growth_bytes=100;handle_growth=1;thread_growth=1;io_read_growth_bytes=1000;private_peak_growth_bytes=200;working_peak_growth_bytes=200};runtime_log_set_sha256=('A'*64);scenario_tree_sha256=('B'*64);save_before_sha256=$saveBefore;save_after_sha256=$saveAfter;header_before_sha256=$headerBefore;header_after_sha256=$headerAfter;controlled_exit=$true;exit_code=0;force_cleanup=$false;fatal_markers=0;stage_path=$stagePath}}
  ...
  $seeds=@([ordered]@{role='frontend';source='private/evidence/M5-013/20260817-013319-c2e7223f/runs/01/user';source_context='immutable-five-race-resource-seed';upstream_result_path='private/evidence/M5-013/20260817-013319-c2e7223f/result.json';upstream_result_sha256='D890A903775A3B53262B9957E7B7DC1D4B76B49B8735360BECD8233842446298';tree_sha256=('E'*64);save_sha256=('C'*64);header_sha256=('8'*64)},[ordered]@{role='gameplay';source='private/save-archive/M6-014/progressed-gameplay-seed/20260829-204551Z-27992E6BC661ABA9-A5A662F728AB46D8';source_context='owner-progressed-save-24-races-23-wins';upstream_result_path='private/save-archive/M6-014/progressed-gameplay-seed/20260829-204551Z-27992E6BC661ABA9-A5A662F728AB46D8/snapshot.json';upstream_result_sha256='9F074F1693C2B42ADF365E7EB4748F0AFF26AFCCCCBB84BA1B357CFC52871DF3';tree_sha256=('F'*64);save_sha256=('D'*64);header_sha256=('9'*64)})
  [ordered]@{schema='mcla-two-hour-soak-suite-v2';task='M6-014';decision='five-two-hour-soak-suite-pass';suite_id='20260819-210000-1234abcd';sdk_version=$sdkVersionPin;sdk_commit=$sdkCommitPin;build_configuration='Release';duration_seconds_per_scenario=7200;sample_interval_seconds=300;capture_interval_seconds=900;game=[ordered]@{file_count=1L;payload_bytes=1L;source_iso_sha256=('7'*64)};seeds=$seeds;build=[ordered]@{clean_build_log_sha256=('2'*64);artifacts=@([ordered]@{name='mcla.exe';sha256=('3'*64)},[ordered]@{name='rexruntime.dll';sha256=('4'*64)},[ordered]@{name='TracyClient.dll';sha256=('5'*64)},[ordered]@{name='rexgpu-xenos.dll';sha256=('6'*64)})};prior_evidence=$priors;scenarios=$scenarios;scope=[ordered]@{five_independent_processes=$false;four_physical_processes=$true;mixed_free_roam_races_session=$true;same_release_artifacts=$true;two_explicit_seed_lineages=$true;same_gameplay_seed_identity=$true;same_seed_identity=$false;frontend_two_hours=$true;free_roam_two_hours=$true;races_two_hours=$true;garage_two_hours=$true;lifecycle_two_hours=$true;monolithic_ten_hour_run_claimed=$false;full_campaign_claimed=$false;music_continuity_claimed=$false}}
}
...
try{
  ...
  ExpectFail wrong-four-part-sdk {param($r)$r.sdk_version=$wrongSdkVersion}
  ExpectFail wrong-sdk-commit {param($r)$r.sdk_commit=$wrongSdkCommit}
  ...
  ExpectFail mixed-five-process-overclaim {param($r)$r.scope.five_independent_processes=$true}
  ExpectFail mixed-four-process-denial {param($r)$r.scope.four_physical_processes=$false}
  ExpectFail mixed-flag-denial {param($r)$r.scope.mixed_free_roam_races_session=$false}
  ExpectFail mixed-flag-type {param($r)$r.scope.mixed_free_roam_races_session='true'}
  ...
  $runnerText=[IO.File]::ReadAllText($runner);$verifierText=[IO.File]::ReadAllText($verify);$watcherText=[IO.File]::ReadAllText((Join-Path $repo 'scripts/watch-soak-save.ps1'));$appText=[IO.File]::ReadAllText((Join-Path $repo 'src/mcla_app.cpp'));$needles=@('DurationSeconds=7200','sampleInterval=300','captureInterval=900','GetProcessIoCounters','CopyFromScreen','GetForegroundWindow','AttachThreadInput','FocusWindow($h)','ForEach-Object{[long]$_.private_bytes}','UtcNow.AddSeconds(20)','did not yield a nontrivial physical capture within 20 seconds','if($activity.Count)','MigrateCompletedStages','header_before_sha256=(Hash','SOAK $($Name.ToUpperInvariant()) READY','RACES <completed> <series-completed>','GARAGE <entries> <committed-changes>','LIFECYCLE <complete-cycles> [LONG]','--xam_user_signin_state=1','--input_backend=sdl','Enter-Gameplay $process $runRoot $user','MCLA_GARAGE_CONTROL v=1 sequence=1 action=START','AUTO START','watch-soak-save.ps1','SAVE WATCHER','-WindowStyle Hidden','WaitForExit(15000)','--sdl_audio_route_audit=true','mcla_audio_route_soak_seconds','Close-Exact','force cleanup','five-two-hour-soak-suite-pass','music_continuity_claimed=$false','private/evidence/M5-013/20260817-013319-c2e7223f/runs/01/user','private/save-archive/M6-014/progressed-gameplay-seed/20260829-204551Z-27992E6BC661ABA9-A5A662F728AB46D8','owner-progressed-save-24-races-23-wins','gameplayRaceCompletions-ne24','gameplayWins-ne23','RebindGameplaySeed','mcla-soak-gameplay-seed-rebind-v1','replace-stale-9-race-9-win-seed-with-owner-progressed-24-race-23-win-seed','aborted-attempts','two_explicit_seed_lineages=$true','same_seed_identity=$false','[IO.Directory]::CreateDirectory($suiteRoot)','RecoverCompletedScenario','Recover-CompletedScenario','interactive wall-clock, resource, capture, and activity journals complete','Frontend audio completion marker is missing.','$pwshHost=(Get-Process -Id $PID).Path','Start-Process $pwshHost','mcla-soak-stage-recovery-v1','WM_CLOSE-to-SDK-hard-exit-0','RecoverMixedRaceCoverage','Recover-MixedRaceCoverage','Read-SaveStatU32','mcla-soak-mixed-race-coverage-v1','mixed_free_roam_races_session','four_physical_processes=$mixed','CMakeLists.txt','ReXGlueCommit','ReXGlueTag','Project SDK pins are missing or inconsistent.','clean exact project-pinned SDK tag and commit');foreach($needle in $needles){if(-not$runnerText.Contains($needle)){throw "Runner source contract missing: $needle"}};if($runnerText.Contains("'Suite root' -Exists")){throw 'An explicitly named new suite must not require a pre-existing root.'};if($runnerText.Contains('Start-Process powershell.exe')){throw 'Save watcher must run under the current PowerShell host, not legacy Windows PowerShell.'}
  ...
  foreach($needle in @('scenarioNames','duration_seconds_per_scenario','sample_count','capture_count','Free-roam region diversity','resource growth exceeds bounds','capture sequence lacks temporal variation','runtime health contract failed','if($s.name-ceq''frontend''){480}else{900}','Result SDK does not match the exact project pin.','SDK identity drifted','Canonical MCLA process remains','Read-SaveStatU32','gRCP','gWIN','operator_series_completed','reviewed_active_race_capture_sha256','PhysicalSoakHours')){if(-not$verifierText.Contains($needle)){throw "Verifier source contract missing: $needle"}}
  [pscustomobject][ordered]@{PositiveFixtures=1;FailClosedNegatives=$negative;SourceContractChecks=$needles.Count+22;SuiteVerified=$true}
