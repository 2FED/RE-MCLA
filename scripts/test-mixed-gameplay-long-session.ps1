...
function BaseResult{
  ...
  [ordered]@{
    schema='mcla-one-hour-mixed-gameplay-long-session-v3';task='M6-014';decision='one-hour-mixed-gameplay-long-session-pass';run_id='20260903-120000-1234abcd';target_duration_seconds=3600;actual_duration_seconds=3600
    ...
    sdk_baseline=[ordered]@{path='private/evidence/M6-014/20260901-153415-d747cf2d/suite.json';sha256='EB553F0B34F00837A7A0C2FA3FDDD4FBEA9444FE4268EAC266D889A80DCA37C6';sdk_version='0.10.0.1';sdk_commit='7dd5cb33002a443b097c0f65d5566c0a0f2db838'}
    project_build=[ordered]@{version='0.8.2.0';configuration='Release';build_log_name='build.log';build_log_sha256=('B'*64)}
    ...
    delivery_prerequisite=[ordered]@{result_path='private/evidence/M6-014/delivery-regressions/20260901-203448-d31ab681/result.json';result_sha256='E7200300CA38718DD6978C26E8F4CE5CF4EF1279441B5F032CE2075F387FF0F1'}
    gameplay_seed=[ordered]@{archive_path='private/save-archive/M6-014/race-back-example';latest_path='private/save-archive/M6-014/race-back-example/latest.json';latest_sha256=('C'*64);captured_utc='2026-09-04T03:52:23.0000000Z';snapshot_directory='20260904-035223Z-012407994331E607-2FAEFBC7FF8CDEAD';snapshot_tree_sha256=('D'*64);save_before_sha256='012407994331E607BFA404F50937B3046747B08D7CE76DAF50DCE51CB74F9C8D';header_before_sha256='2FAEFBC7FF8CDEADBCE025D9BE914436ED05E27D041F72C8C88798612CFB0D60';save_after_sha256=('1'*64);header_after_sha256=('2'*64);working_copy_tree_sha256=('A'*64)}
    ...
    release_artifacts=@([ordered]@{name='mcla.exe';sha256=('3'*64)},[ordered]@{name='mcla_crash_handler.exe';sha256=('4'*64)},[ordered]@{name='rexruntime.dll';sha256=('5'*64)},[ordered]@{name='TracyClient.dll';sha256=('6'*64)},[ordered]@{name='rexgpu-xenos.dll';sha256=('7'*64)})
...
try{
  ...
  ExpectFail bad-sdk-baseline {param($r)$r.sdk_baseline.sha256=('0'*64)}
  ExpectFail bad-project-version {param($r)$r.project_build.version='0.8.1.0'}
  ...
  ExpectFail bad-delivery-prerequisite {param($r)$r.delivery_prerequisite.result_sha256=('0'*64)}
  ExpectFail bad-gameplay-seed {param($r)$r.gameplay_seed.save_before_sha256='bad'}
  ExpectFail escaped-gameplay-seed {param($r)$r.gameplay_seed.archive_path='private/save-archive/M5-009/bad';$r.gameplay_seed.latest_path='private/save-archive/M5-009/bad/latest.json'}
  ExpectFail bad-working-seed-copy {param($r)$r.gameplay_seed.working_copy_tree_sha256='bad'}
  ...
  $verifierNeedles=@('Exactly thirteen resource samples are required.','Exactly five capture attempts are required.','Captured frame count is outside the accepted range.','Captured frames do not match successful attempts.','Actual duration is not bound to the final sample.','Current physical artifact, game, or SDK baseline identity drifted.','Historical and current executable identities must remain distinct.','verify-delivery-transition-regression.ps1','Physical gameplay seed drifted.','Project build log drifted.','Final archive physical snapshot drifted.','contains a reparse point.','Canonical MCLA process remains.','ConvertTo-Json $persistedSamples -Depth 15 -Compress','$hostAudit=','Title terminated; hard-exiting process.')
  ...
  if($verifierText.Contains("'Execution complete'")){throw 'Verifier requires an obsolete shutdown marker.'}
  if($verifierText.Contains('$host=')){throw 'Verifier assigns the read-only PowerShell Host automatic variable.'}
  if($verifierText.Contains("Safe 'VERSION' 'Project version'")){throw 'Verifier incorrectly binds immutable evidence to the current repository version.'}
  [pscustomobject][ordered]@{PositiveFixtures=2;FailClosedNegatives=$negative;SourceContractChecks=$runnerNeedles.Count+$verifierNeedles.Count+6;OneHourGateVerified=$true}
