...
$evidence = Safe 'private/evidence/M4-011' 'Evidence root'; [IO.Directory]::CreateDirectory($evidence) | Out-Null
...
Write-Host 'M4-011 [1/7]: validating pinned post-OOBE save and source-game integrity...' -ForegroundColor Cyan
$gameBefore = Game $game
Write-Host 'M4-011 [2/7]: clean-building and installing ReXGlue SDK...' -ForegroundColor Cyan
...
Write-Host 'M4-011 [3/7]: running focused VFS content-root tests...' -ForegroundColor Cyan
...
Write-Host 'M4-011 [4/7]: clean-building the MCLA host...' -ForegroundColor Cyan
...
Write-Host 'M4-011 [5/7]: running 3 isolated saved-gameplay -> pause -> options routes...' -ForegroundColor Cyan
$records = @()
for ($index = 1; $index -le 3; $index++) {
    ...
    } catch { $failure = $_; if ($process -and -not $process.HasExited) { $forced = $true; Stop-Process -Id $process.Id -Force -ErrorAction SilentlyContinue; $null = $process.WaitForExit(5000) }; if ($forced) { throw "M4-011 failure required force cleanup. $($failure.Exception.Message) Private run: '$root'." }; throw }
    $probe = & $verify -ProbeOnly -RuntimeLogPath $log -UserRoot $user; $tree = Tree $cycle
    ...
    Write-Host "M4-011 cycle ${name}: saved gameplay, pause, Options, and external close PASS." -ForegroundColor Green
}

Write-Host 'M4-011 [6/7]: checking immutable source/runtime/seed state...' -ForegroundColor Cyan
...
Write-Host 'M4-011 [7/7]: final physical/result verification...' -ForegroundColor Cyan
