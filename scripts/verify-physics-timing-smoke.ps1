...
$sdkEvidenceCommits=@{'0.9.0.19'='53c16fcfcbfee83752b7689cf74aba1d69a185fa';'0.9.0.20'='c4aa30c35386bb4d2ef051a59ea8e71bab667172'}
if(($record.PSObject.Properties.Name-join',')-cne($properties-join',')-or$record.schema-cne'mcla-physics-timing-v1'-or$record.task-cne'M5-008'-or$record.decision-cne'stock-30-fixed-step-and-real-time-throughput-pass'-or$record.cycle_count-ne3-or-not($record.cycle_count-is[int]-or$record.cycle_count-is[long])-or$record.fixed_step_microseconds-ne33333-or-not($record.fixed_step_microseconds-is[int]-or$record.fixed_step_microseconds-is[long])-or-not$sdkEvidenceCommits.ContainsKey([string]$record.sdk_version)-or$record.sdk_commit-cne$sdkEvidenceCommits[[string]$record.sdk_version]-or$record.build_configuration-cne'Release'-or-not($record.stock_30_target-is[bool])-or$record.stock_30_target-ne$true-or-not($record.stock_speed_sustained-is[bool])-or$record.stock_speed_sustained-ne$true-or-not($record.actual_output_30fps_sustained-is[bool])-or$record.actual_output_30fps_sustained-ne$true){throw 'Result schema, types, or decision are invalid.'}
...
if((ConvertTo-Json $record.artifacts_before -Compress -Depth 4)-cne(ConvertTo-Json $record.artifacts_after -Compress -Depth 4)){throw 'Evidence runtime artifacts drifted.'}
if($record.sdk_version-ceq'0.9.0.20'){$artifacts=@(Get-ArtifactIdentity (Join-Path $repo 'out/build/win-amd64-release'));if((ConvertTo-Json $record.artifacts_after -Compress -Depth 4)-cne(ConvertTo-Json $artifacts -Compress -Depth 4)){throw 'Current runtime artifact identity mismatch.'}}
else{$historicalArtifacts=@([pscustomobject][ordered]@{name='mcla.exe';sha256='2023116B35890B4D014CFB66458E2FDEBBC0082114C330022D7600FA6570AB1E'},[pscustomobject][ordered]@{name='rexruntime.dll';sha256='44519ED0081DBD79B6B6C007E67E5CE0232E819FA3585497A505E1DB3E9163EB'},[pscustomobject][ordered]@{name='TracyClient.dll';sha256='742EEECE19B8A8555B5E8A707FA0D0462678D2802021911E016AA3909E5911BE'},[pscustomobject][ordered]@{name='rexgpu-xenos.dll';sha256='114C38071D7AA4177987D232211AE39399E0BE267C95CF7270D886929AE49550'});if((ConvertTo-Json $record.artifacts_after -Compress -Depth 4)-cne(ConvertTo-Json $historicalArtifacts -Compress -Depth 4)){throw 'Historical runtime artifact identity mismatch.'};git -C (Join-Path $repo 'third_party/rexglue-sdk') diff --quiet v0.9.0.19 v0.9.0.20 -- include/rex/ui/presenter.h src/ui/presenter.cpp src/graphics/graphics_system.cpp;if($LASTEXITCODE-ne0){throw 'Current SDK changed the accepted timing-observability implementation.'}}
...
$sdkHead=(& git -C (Join-Path $repo 'third_party/rexglue-sdk') rev-parse HEAD).Trim();if($LASTEXITCODE-ne0-or$sdkHead-cne'576b34fd233acf4579dd2375691dbe86fb4bf8e1'){throw 'Current SDK commit identity mismatch.'}
